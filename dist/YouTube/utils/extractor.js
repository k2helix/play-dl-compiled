...
async function video_basic_info(url, options = {}) {
    ...
    const musicInfo = initial_response.contents.twoColumnWatchNextResults.results.results.contents?.[1]?.videoSecondaryInfoRenderer
        ?.metadataRowContainer?.metadataRowContainerRenderer?.rows;
    const music = [];
    if (musicInfo) {
        musicInfo.forEach((x) => {
            if (!x.metadataRowRenderer)
                return;
    ...
    const video_details = new Video_1.YouTubeVideo({
        ...
        views: vid.viewCount,
        tags: vid.keywords,
        likes: parseInt(initial_response.contents.twoColumnWatchNextResults.results.results.contents
