...
async function spotify(url) {
    ...
    if (url_.indexOf('track/') !== -1) {
        ...
        return new classes_1.SpotifyTrack(resObj);
    }
        ...
        if (resObj.error)
            throw new Error(`Got ${resObj.error.status} from the spotify request: ${resObj.error.message}`);
        return new classes_1.SpotifyAlbum(resObj, spotifyData, false);
    }
        ...
        if (resObj.error)
            throw new Error(`Got ${resObj.error.status} from the spotify request: ${resObj.error.message}`);
        return new classes_1.SpotifyPlaylist(resObj, spotifyData, false);
    }
    else
